...
def InpaymentDownload(request,data):
    pdf_files = []
    for PermitId in data.split(','):
        ...
        try:
            InwardCarrier = InwardCarrierAgent.objects.get(
                code=Inheader.InwardCarrierAgentCode)
        ...
        if Inheader.PermitNumber == "":
            draft = "DRAFT"
            inpstrtdate = ""
            inpEndate = ""
        else:
            draft = Inheader.PermitNumber
            barcode = code39.Standard39(draft, barHeight=36.0, barWidth=1.1, baseline=9.0,
        ...
        p.drawString(lftcol, 660, "DECLARATION TYPE  : " +
                    (str(Inheader.DeclarationType)[6:]).upper())
        ...
        p.drawString(rgtcol, rgy, ("TOTAL GROSS WT/UNIT  : {:>18}/{}").format(
            totalGross, Inheader.TotalGrossWeightUOM))
        rgy -= 10
        p.drawString(rgtcol, rgy, ("TOTAL OUTER PACK/UNIT: {:>18}/{}").format(
            Inheader.TotalOuterPack, Inheader.TotalOuterPackUOM))
        rgy -= 10
        p.drawString(rgtcol, rgy, ("TOT EXCISE DUT PAYABLE  : S${:>17}").format(
            Inheader.TotalExDutyAmt))
        rgy -= 10
        p.drawString(rgtcol, rgy, ("TOT CUSTOMS DUT PAYABLE : S${:>17}").format(
            Inheader.TotalCusDutyAmt))
        rgy -= 10
        p.drawString(rgtcol, rgy, ("TOT OTHER TAX PAYABLE   : S${:>17}").format(
            Inheader.TotalODutyAmt))
        rgy -= 10
        p.drawString(rgtcol, rgy, ("TOTAL GST AMT           : S${:>17}").format(
            Inheader.TotalGSTTaxAmt))
        rgy -= 10
        p.drawString(rgtcol, rgy, ("TOTAL AMOUNT PAYABLE    : S${:>17}").format(
            Inheader.TotalAmtPay))
        rgy -= 10
        p.drawString(rgtcol, rgy, ("CARGO PACKING TYPE: {} ").format(
            ((Inheader.CargoPackType)[3:]).upper()))
        rgy -= 10
        ...
        rgy -= 10
        p.drawString(rgtcol, rgy, ("CONVEYANCE REFERENCE NO: {} ").format(
            CoveyanceNo.upper()))
        rgy -= 10
        ...
        rgy -= 10
        p.drawString(rgtcol, rgy, "ARRIVAL DATE         : {}".format(
            (Inheader.ArrivalDate).strftime('%d/%m/%Y')))
        rgy -= 10
        ...
        for item in ItemDtl.objects.filter(PermitId=PermitId).order_by('ItemNo'):
            ...
            itemy = itemyF(itemy)
            if '--SELECT--' != (item.Making).upper():
                p.drawString(makingx, itemy, ((item.Making)[:2]).upper())
            ...
            def CascFunc(sno, cascn):
                ...
                for casc in Cascdtl.objects.filter(ItemNo=item.ItemNo, PermitId=PermitId, CascId=cascn):
                    ...
                    if str(casc.Quantity) != "0.0000":
                        p.drawString(lftcol+280, itemy, ('{:10d}.{} {}').format(
                            int(casc.Quantity), str(casc.Quantity).split(".")[1], casc.ProductUOM))
                    itemy = itemyF(itemy)
        ...
        if draft == "DRAFT":
            ...
            filename = f"/Users/hightech/Desktop/Yosuva_KttProject/DownloadCcp/{draft}1.pdf" #f"D:/Users/Public/PDFFilesKtt/1{draft}.pdf"
        else:
            filename = f"/Users/hightech/Desktop/Yosuva_KttProject/DownloadCcp/{draft}1.pdf" #f"D:/Users/Public/PDFFilesKtt/1{draft}.pdf"
        p.setTitle(draft)
        p.save()
        ...
        output_stream = open(f"/Users/hightech/Desktop/Yosuva_KttProject/DownloadCcp/{draft}.pdf", "wb")#f"D:/Users/Public/PDFFilesKtt/{draft}.pdf
        output.write(output_stream)
        output_stream.close()
        pdf_files.append(f"/Users/hightech/Desktop/Yosuva_KttProject/DownloadCcp/{draft}.pdf")
